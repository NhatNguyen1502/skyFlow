...
object ErrorHandlingService {
   ...
   */
  def processWithSupervision(inputPath: Path, outputPath: Path)
                            (implicit system: ActorSystem[_]): Future[IOResult] = {

    val decider: Supervision.Decider = {
      case _: NumberFormatException  => Supervision.Resume   // bỏ qua dòng CSV lỗi số
      case _: IllegalArgumentException => Supervision.Resume // bỏ qua dữ liệu invalid
  ...
  /**
   * Thay vì throw exception, wrap mọi kết quả trong Either[Error, Success].
   * An toàn hơn supervision vì:
   ...
   */
  def processWithEitherPattern(inputPath: Path, outputPath: Path, errorPath: Path)
    ...
    // Validate an toàn
    val safeValidate: Flow[Either[StreamError, BookingRecord], Either[StreamError, BookingRecord], NotUsed] =
  ...
  /**
   * Dùng GraphDSL + Partition để chia stream thành N nhánh rõ ràng.
   * Advanced hơn divertTo — cho phép > 2 nhánh.
   */
  def processWithDLQ(inputPath: Path, outputPath: Path, dlqPath: Path)
                    (implicit system: ActorSystem[_]): Future[ErrorHandlingReport] = {
    ...
    RunnableGraph.fromGraph(GraphDSL.create() { implicit builder =>
      ...
      val partition = builder.add(
        Partition[Either[StreamError, BookingRecord]](2, {
          case Left(_)  => 0
          case Right(_) => 1
        })
      )

      // Error branch → DLQ file
      val errorFlow = builder.add(
        Flow[Either[StreamError, BookingRecord]]
          .collect { case Left(err) => err }
          .map(e => ByteString(s"${e.line.getOrElse("")}|${e.message}\n"))
      )
      val dlqSink = builder.add(FileIO.toPath(dlqPath))

      // Success branch → output file
      val successFlow = builder.add(
        Flow[Either[StreamError, BookingRecord]]
          .collect { case Right(record) => record }
          .map(r => ByteString(r.toCsvLine + "\n"))
      )
      val outputSink = builder.add(FileIO.toPath(outputPath))

      // Wiring
      source ~> partition
                partition.out(0) ~> errorFlow   ~> dlqSink
                partition.out(1) ~> successFlow ~> outputSink

      ClosedShape
    }).run()

    // Note: Graph trả về NotUsed. Để lấy report, cần thêm monitoring.
    Future.successful(ErrorHandlingReport(0, 0))
  }
   ...
   * Hữu ích cho: DB connection lost, network timeout, external API errors.
   */
  def restartableProcessing(inputPath: Path)
   ...
   */
  def processWithRecover(inputPath: Path)
                        (implicit system: ActorSystem[_]): Future[Seq[BookingRecord]] = {
  ...
  private def validateUnsafe(record: BookingRecord): BookingRecord =
    BookingValidator.validate(record) match {
      ...
      case Left(error)  => throw new IllegalArgumentException(error.violations.mkString(", "))
    }
}
