...
object BackpressureService {
  ...
  def processWithThrottle(inputPath: Path, elementsPerSecond: Int = 100)
                         (implicit system: ActorSystem[_]): Future[ThrottleReport] = {
    ...
    readCsvSource(inputPath)
      ...
      .map { count =>
        ...
        ThrottleReport(
          ...
          effectiveRate = if (durationMs > 0) count * 1000.0 / durationMs else 0.0
        )
      }
  }
   ...
   */
  def processWithBuffer(inputPath: Path, bufferSize: Int, strategy: OverflowStrategy)
  ...
  /**
   * So sánh 3 strategies phổ biến trên cùng 1 input.
   * Khi chạy xong, so sánh totalProcessed: backpressure sẽ xử lý hết,
   * dropHead/dropNew có thể mất data.
   */
  def compareBufferStrategies(inputPath: Path)
                             (implicit system: ActorSystem[_]): Future[Map[String, Int]] = {

    implicit val ec = system.executionContext

    for {
      backpressure <- processWithBuffer(inputPath, 100, OverflowStrategy.backpressure)
  ...
  /**
   * conflate: Khi consumer chậm hơn producer, GOM nhiều elements thành 1 summary.
   * Không mất data — data được merge/aggregate thay vì drop.
   *
   * Ví dụ: 1000 booking records → consumer chỉ kịp xử lý 100 summaries.
   */
  def processWithConflate(inputPath: Path)
                         (implicit system: ActorSystem[_]): Future[Seq[RouteSummary]] = {

    readCsvSource(inputPath)
      .conflateWithSeed(record => RouteSummary(record.routeKey, 1, record.seatsRequested)) {
        (summary, record) =>
   ...
   */
  def benchmarkThroughput(inputPath: Path, reportEvery: Int = 100)
                         (implicit system: ActorSystem[_]): Future[ThrottleReport] = {
    ...
    readCsvSource(inputPath)
      ...
      .wireTap { case (_, idx) =>
        if ((idx + 1) % reportEvery == 0) {
          ...
          println(f"[Progress] Processed ${idx + 1} records, rate: $rate%.1f records/sec")
        }
      }
  ...
  private def simulateSlowProcessing(record: BookingRecord)
                                    (implicit system: ActorSystem[_]): Future[BookingRecord] = {
    akka.pattern.after(10.millis)(Future.successful(record))(system.classicSystem)
  }

  private def simulateSlowSummaryProcessing(summary: RouteSummary)
                                           (implicit system: ActorSystem[_]): Future[RouteSummary] = {
    akka.pattern.after(50.millis)(Future.successful(summary))(system.classicSystem)
  }
}
