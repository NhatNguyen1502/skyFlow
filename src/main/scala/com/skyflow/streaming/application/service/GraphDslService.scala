 ...
 * - Broadcast[T] — 1 input → N outputs (clone mọi element tới tất cả outputs)
...
object GraphDslService {
  ...
  def broadcastPipeline(inputPath: Path, outputPath: Path)
    ...
    RunnableGraph.fromGraph(GraphDSL.create(countSink) { implicit builder => countMat =>
  ...
  /**
   * Balance: phân phối elements ROUND-ROBIN tới N workers.
   * Merge: gom kết quả từ N workers lại thành 1 stream.
   *
   * Khác với Broadcast: Balance gửi mỗi element tới CHỈ 1 worker.
   * → Dùng để parallel processing khi mỗi element chỉ cần xử lý 1 lần.
   */
  def balancedWorkers(inputPath: Path, workerCount: Int = 4)
                     (implicit system: ActorSystem[_]): Future[Seq[String]] = {

    RunnableGraph.fromGraph(GraphDSL.create(Sink.seq[String]) { implicit builder => seqMat =>
      import GraphDSL.Implicits._
   ...
   */
  def partitionByType(inputPath: Path, domesticOutputPath: Path, otherOutputPath: Path)
                     (implicit system: ActorSystem[_]): Future[Done] = {

    val domesticRoutes = Set("HAN_SGN", "HAN_DAD", "SGN_DAD", "SGN_PQC", "HAN_CXR", "DAD_PQC")

    RunnableGraph.fromGraph(GraphDSL.create() { implicit builder =>
      import GraphDSL.Implicits._

      val source = builder.add(csvSource(inputPath))
      val partition = builder.add(Partition[BookingRecord](2, record =>
        if (domesticRoutes.contains(record.routeKey)) 0 else 1
      ))
   ...
   *
   * Ví dụ: Zip booking records với sequential IDs.
   * Lưu ý: Zip chờ cả 2 inputs → stream chậm theo source chậm nhất.
   */
  def zipWithSequence(inputPath: Path)
                     (implicit system: ActorSystem[_]): Future[Seq[(Long, BookingRecord)]] = {

    RunnableGraph.fromGraph(
      GraphDSL.create(Sink.seq[(Long, BookingRecord)]) { implicit builder => seqMat =>
        import GraphDSL.Implicits._

        val csvSrc = builder.add(csvSource(inputPath))
        val idSrc = builder.add(Source.fromIterator(() => Iterator.from(1).map(_.toLong)))
  ...
  /**
   * Tạo Flow có 1 input và 2 outputs (success + error).
   * Đây là ví dụ custom Shape — ngoài ClosedShape tiêu chuẩn.
   *
   * Trả về Flow[String, BookingRecord, NotUsed] kèm error output.
   */
  def parseWithErrorBranch: Graph[FanOutShape2[String, BookingRecord, String], NotUsed] = {
    GraphDSL.create() { implicit builder =>
      import GraphDSL.Implicits._

      val parse = builder.add(Flow[String].map(CsvParser.parseLine))
      val partition = builder.add(Partition[Either[Any, BookingRecord]](2, {
        case Left(_)  => 0
        case Right(_) => 1
      }))

      val extractError = builder.add(
        Flow[Either[Any, BookingRecord]].collect { case Left(err) => err.toString }
      )
      val extractSuccess = builder.add(
        Flow[Either[Any, BookingRecord]].collect { case Right(record) => record }
      )
